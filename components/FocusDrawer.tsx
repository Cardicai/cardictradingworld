...
const FOCUS_ITEMS = ['AI Mentor', 'Tools', 'Club', 'Game', 'Funding', 'NexLink'] as const

export default function FocusDrawer() {
  const focusMenuOpen = useUI((state) => state.focusMenuOpen)
  const closeFocusMenu = useUI((state) => state.closeFocusMenu)
  ...
  }, [focusMenuOpen, closeFocusMenu])

  useEffect(() => {
    const previous = prevPath.current
    if (previous !== pathname && focusMenuOpen) {
      closeFocusMenu()
    }
    prevPath.current = pathname
  }, [pathname, focusMenuOpen, closeFocusMenu])

  const buttons = useMemo(() => {
    const lookup = new Map(NAV_ITEMS.map((item) => [item.label, item]))
    return FOCUS_ITEMS.map((label) => lookup.get(label)).filter(
  ...
  return (
    <>
      <div
        aria-hidden={!focusMenuOpen}
        onClick={focusMenuOpen ? close : undefined}
        className={cx(
          'fixed inset-0 z-[70] bg-black/55 transition-opacity duration-300 sm:bg-black/65',
          focusMenuOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'
        )}
      />
      <aside
        ref={drawerRef}
        aria-hidden={!focusMenuOpen}
        aria-modal={focusMenuOpen}
        role="dialog"
        id="focus-drawer"
        className={cx(
          'focus-drawer pointer-events-auto fixed inset-y-0 right-0 z-[80] flex w-full max-w-[360px] translate-x-full flex-col',
      ...
      >
        ...
        <nav className="flex flex-1 flex-col gap-4 overflow-y-auto px-6 pb-10">
          {buttons.map((item) => {
            ...
            const classes =
              'focus-drawer__button focus-visible:ring-offset-0 w-full min-h-[3.25rem] rounded-2xl border border-cyan-400/30 bg-gradient-to-br from-cyan-500/15 via-transparent to-violet-500/15 px-4 py-5 text-left transition hover:border-cyan-300/60 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70'
            ...
            return (
              <Link key={item.label} href={item.href} className={classes} onClick={close}>
                {content}
              </Link>
            )
          })}
